 ...
 * @author David
 */
public class GroupUserRenderer extends JPanel implements ListCellRenderer<User>{
    ...
    public GroupUserRenderer(Group group) {
        this.group = group;
    ...
    public Component getListCellRendererComponent(JList<? extends User> list, User user, int index, boolean isSelected, boolean cellHasFocus) {
        ...
        if(group.isAdmin(user)){
            nameLabel.setText("<html>" + user.getName() + "<br/>(" + user.getId() + ") <b>(Moderador)</b></html>");
        } else {
